    ...
    if(which.min(AIC3)==1){
      ...
      while(length(coef3)>0){
        ...
        for (i in 1:length(coef3)){
          ...
          test3.specs[[j]]= c(test3.specs[[min.AIC3]],0,names2specs(coef3[i], values=values))
...
rownames(results) = c("n000", "est.pop.size", "model")
...
mean(unlist(results$n000)) #7845.11
mean(unlist(results$est.pop.size))
sd(unlist(results$est.pop.size))/(MC^0.5)
# clearly biased 
...
length(which(as.vector(results[,4])=="G W Z B F S ZS WF ZF GF FS ZSF"))
length(which(as.vector(results[,4])=="G W Z B F S ZS GF ZF WF FS ZSF"))
