  ...
  if(which.min(AIC2)==1){
    ...
    while(length(coef2)>0){
      ...
      if ((min.AIC2 == which.min(AIC2))&(length(which(AIC2== AIC2[min.AIC2]))>1)){
        ...
        # if we did a new loop and the last.min is still the same, we stop (taking out coefs didn't improve the model)
...
# pretty close with a mean over 541 iterations of 21988 (over estimation by 2000)
# standard deviation of 1416
...
alias(start.FIT$tb.fit.glm)
# the multicollinearity explains why some coefficients appear as NA :
# they are perfectly predicted by combining other variables

############################################################################
# G:Z:F + G:Z:S + G:Z:B + G:W:S + G:W:B + G:F:S + G:F:B + G:S:B + Z:W:F + Z:W:B + Z:F:S + Z:F:B + Z:S:B + W:F:S + W:F:B + W:S:B
# G:Z:F + G:Z:S + G:Z:B + G:W:S + G:W:B + G:F:S + G:F:B + G:S:B + Z:W:F + Z:W:B + Z:F:S + Z:F:B + Z:S:B + W:F:S + W:F:B + F:S:B