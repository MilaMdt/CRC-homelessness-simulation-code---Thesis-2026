...
# inclusion probability based on the covariates and samples
...
for(mc in 1:MC){
  ...
  model.norm          	<- data.frame(nr=integer(),
                                   ...
                                   model=character(),
                                   specs=character())
colnames(model.norm)	<- c("nr","DEV","df","BIC","nHat0","model","specs")
...
for(zz in 1:n.GWZ.models){
  ...
  colnames(cov.model.norm)	<- c("nr","DEV","df","BIC","nHat0")
  ...
  rang   			<- round(rank(identified.cov.models[,4]),0) # gives the position from lower value to larger for BIC
  cov.model.nr		 	<- subset(identified.cov.models[,1],rang==1) # number of the best model regarding BIC
  ...
  for (j in 1:n.models){
    ...
    model.norm[j,]$BIC=round(FIT$norm.bic,3)
...
best2.model
BIC2[which.min(BIC2)] # in 2023 we get a model with a BIC of
...
sd(results[,"N_est_standard_cov"])/(MC^0.5)
