...
for(mc in 1:MC) {
  ...
  if(length(coef2)<(length(twoe)-2)){
    ...
    # only consider the 3-way terms derived from newcoef
      ...
      if(which.min(BIC4)==1){
        ...
        while(length(coef4)>0){
          ...
          for (i in 1:length(coef4)){
            ...
            test4.specs[[j]]= c(test4.specs[[min.BIC4]],0,names2specs(coef4[i], values=values))
